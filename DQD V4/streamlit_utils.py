...
def create_errors_excel(results, original_df, column_mappings):
    ...
    Each sheet shows the failed claim numbers + relevant column values.
    original_df: the raw (pre-rename) dataframe so we can look up actual values.
    column_mappings: logical_name -> physical_col_name
    """
    output = io.BytesIO()

    # Build reverse mapping: logical -> physical
    rev = {k: v for k, v in column_mappings.items()}
    claim_col = rev.get('claim_number')  # physical column name for claim number

    # Rename original_df to logical names for easy lookup
    df = original_df.rename(columns={v: k for k, v in column_mappings.items()})

    # Columns to show per check type for context
    check_context_cols = {
        ...
        'missing_claims_from_prev_month':None,   # failed_records are claim numbers directly
        'total_paid_non_decreasing_vs_prev': None,  # same
        'risk_policy_match':             None,
    }
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        for category, subcategories in results['detailed_results'].items():
            for subcategory, checks in subcategories.items():
                for check in checks:
                    ...
                    if ctx_cols is None:
                        # failed_records are identifiers (claim numbers / policy numbers) directly
                        sheet_df = pd.DataFrame({'Failed Record': failed_records})
                    else:
                        # failed_records are row indices into df
                        try:
                            subset = df.loc[failed_records].copy()
                        except Exception:
                            sheet_df = pd.DataFrame({'Failed Row Index': failed_records})
                            sheet_df.to_excel(writer, sheet_name=sheet_name, index=False)
                            sheets_written += 1
                            continue

                        # For checks with a single 'check_field', substitute actual field name
                        ...
                        if cols_to_show:
                            sheet_df = subset[cols_to_show].reset_index(drop=True)
                        else:
        ...
        if sheets_written == 0:
            # No errors at all — write a placeholder sheet
...
def load_default_config():
    ...
    config = {
        'checks': [
            ...
            {'id': '3.3', 'name': 'Total Incurred = Total Paid + Total OS', 'type': 'incurred_equals_paid_plus_os', 'tolerance': 0, 'category': 'Logical', 'subcategory': 'Rules', 'points': 10, 'enabled': True},
