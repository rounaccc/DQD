...
def _points_from_pass_rate(points_possible: float, pass_rate: float,
                           thresh_full: float = 95.0, thresh_mid: float = 90.0,
                           mid_points: float = 3.0) -> float:
    if points_possible <= 0:
        return 0.0
    pr = float(pass_rate) if pass_rate is not None else 0.0
    pr = max(0.0, min(100.0, pr))
    if pr > thresh_full:
        return float(int(points_possible))
    elif pr > thresh_mid:
...
class DataQualityValidator:
    ...
    def validate(
        ...
        # pandas only renames it to one. Copy data to all missing logical names.
    ...
    def _run_one_check(self, check: Dict[str, Any], ctx: CheckContext) -> Dict[str, Any]:
        ...
        try:
            ...
            points_earned = _points_from_pass_rate(
                points_possible, pass_rate,
                thresh_full=float(ctx.settings.get("thresh_full", 95)),
                thresh_mid=float(ctx.settings.get("thresh_mid", 90)),
                mid_points=float(ctx.settings.get("mid_points", 3)),
            )
