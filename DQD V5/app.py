    ...
    if uploaded_config_early:
        try:
            ...
            if isinstance(loaded, dict) and ("metadata" in loaded) and ("column_mappings" in loaded) and ("checks" in loaded):
                ...
                # Restore scoring thresholds if saved in config
                scoring = loaded.get("scoring_thresholds", {})
                st.session_state.thresh_full = scoring.get("thresh_full", 95)
                st.session_state.thresh_mid = scoring.get("thresh_mid", 90)
                st.session_state.mid_points = scoring.get("mid_points", 3)

                st.success("✓ Config loaded — all steps pre-filled. Walk through to review and modify anything.")
            else:
...
# ============================================================================
    ...
    closed_csv = st.text_input("Closed status values (comma-separated)", value=", ".join(getattr(st.session_state, "closed_status_values", ["Closed"])))
    st.session_state.closed_status_values = [v.strip() for v in closed_csv.split(",") if v.strip()]

    st.markdown("---")
    st.subheader("📊 Scoring Thresholds")
    st.caption("Points are awarded in 3 tiers based on pass rate.")

    col_t1, col_t2, col_t3 = st.columns(3)
    with col_t1:
        thresh_full = st.number_input("Full points if pass rate >", min_value=50, max_value=100, value=int(getattr(st.session_state, 'thresh_full', 95)), step=1, help="Pass rate above this % earns full points")
    with col_t2:
        thresh_mid = st.number_input("Partial points if pass rate >", min_value=50, max_value=100, value=int(getattr(st.session_state, 'thresh_mid', 90)), step=1, help="Pass rate above this % (but below full threshold) earns partial points")
    with col_t3:
        mid_points = st.number_input("Partial points awarded", min_value=0, max_value=100, value=int(getattr(st.session_state, 'mid_points', 3)), step=1, help="Fixed points awarded when pass rate is in the middle tier")

    if thresh_mid >= thresh_full:
        st.warning("⚠️ Partial threshold must be lower than full threshold.")

    st.caption(f"Summary: pass rate > {thresh_full}% → full points | > {thresh_mid}% → {mid_points} pts | else → 0 pts")

    st.session_state.thresh_full = thresh_full
    st.session_state.thresh_mid = thresh_mid
    st.session_state.mid_points = mid_points
    ...
    if st.session_state.validation_results is None:
        with st.spinner("🔍 Running validation checks..."):
            ...
            st.session_state.config["_settings"] = {
                ...
                "thresh_full": getattr(st.session_state, "thresh_full", 95),
                "thresh_mid": getattr(st.session_state, "thresh_mid", 90),
                "mid_points": getattr(st.session_state, "mid_points", 3),
            }
            ...
            # Inject source metadata so db_utils can store it
            results['metadata'] = {
                'source_database': getattr(st.session_state, 'claims_db_selected', getattr(st.session_state, 'claims_db', None)),
                'source_table': getattr(st.session_state, 'claims_table_selected', getattr(st.session_state, 'claims_table', None)),
                'bdx_month': str(getattr(st.session_state, 'bdx_month_selected', getattr(st.session_state, 'bdx_month', ''))),
            }
